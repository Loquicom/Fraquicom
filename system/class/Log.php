...
class Log {
     ...
     * @throws LogException
     */
    public function __construct($name, $path = './log.txt', $keep = true, $trace = true) {
        if (trim($name) == '') {
    ...
    /**
     * Ecris le contenue dans le fichier
    ...
    public function writeLog($nom = '', $sendMail = false, $close = true) {
        ...
        //Si il n'y a rien à écrire
        if (empty($this->content) || (count($this->content) == 1 && isset($this->content['initial']) && trim($this->content['initial']) == '')) {
            //Fin de la methode
            return;
        }
        //Ouverture du fichier
    ...
    public function sendMail($mail = '', $function = null) {
        ...
        if ($function !== null || self::$emailFunction !== null) {
            ...
            //Aucune fonction d'email parametrée
            return false;
        }
    }
